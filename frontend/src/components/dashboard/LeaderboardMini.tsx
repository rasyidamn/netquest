...
import { IconTrophy, IconMedal, IconCrown, IconFlame } from "@tabler/icons-react";
...
function getRankIcon(rank: number) {
   if (rank === 1) return <IconCrown size={22} className="text-yellow-400 drop-shadow-[0_0_8px_rgba(250,204,21,0.6)]" />;
   if (rank === 2) return <IconMedal size={22} className="text-slate-300 drop-shadow-[0_0_5px_rgba(203,213,225,0.5)]" />;
   if (rank === 3) return <IconMedal size={22} className="text-amber-600 drop-shadow-[0_0_5px_rgba(217,119,6,0.5)]" />;
...
export function LeaderboardMini({ isLoading, entries, currentUserName }: LeaderboardMiniProps) {
   ...
   return (
      <div className="card relative overflow-hidden bg-base-200/40 backdrop-blur-xl border border-white/10 shadow-lg group h-full">
         ...
         <div className="absolute -top-16 -right-16 w-32 h-32 bg-warning/10 rounded-full blur-[40px] pointer-events-none transition-transform duration-700 group-hover:scale-150" />

         <div className="card-body p-6 relative z-10">
            <div className="flex items-center justify-between mb-6">
               <h3 className="text-sm font-bold uppercase tracking-[0.2em] text-base-content/60 flex items-center gap-2">
                  <IconFlame size={18} className="text-warning/80 animate-pulse" />
                  Papan Peringkat
               </h3>
            </div>
