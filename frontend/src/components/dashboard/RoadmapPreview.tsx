...
import { IconLock, IconCheck, IconRocket, IconArrowRight, IconMapRoute } from "@tabler/icons-react";
...
function getStatusIcon(status: "LOCKED" | "ACTIVE" | "COMPLETED") {
   switch (status) {
      case "LOCKED":
         return <IconLock size={18} className="text-base-content/30" />;
      case "ACTIVE":
         return <IconRocket size={18} className="text-primary drop-shadow-[0_0_5px_rgba(var(--color-primary),0.8)]" />;
      case "COMPLETED":
         return <IconCheck size={18} stroke={3} className="text-success" />;
   }
}
         ...
         <div className="card-body p-6 relative z-10">
            <div className="flex items-center justify-between mb-6">
               <h3 className="text-sm font-bold uppercase tracking-[0.2em] text-base-content/60 flex items-center gap-2">
                  <IconMapRoute size={18} className="text-primary/70" />
                  Log Perjalanan
               </h3>
               <button 
               ...
               >
                  Lihat Peta <IconArrowRight size={14} />
               </button>
            </div>
                        ...
                        >
                           ...
                           <div 
                              className={cn(
                                 ...
                                 isActive ? "border-primary text-primary shadow-[0_0_10px_rgba(var(--color-primary),0.3)]" 
                                 : isCompleted ? "border-success text-success bg-success/10"
                                 : "border-base-300 text-base-content/30"
                              )}
                           >
                              {isActive ? <span className="absolute inset-0 rounded-full animate-ping bg-primary/20" /> : null}
                              {index + 1}
                           </div>
                           ...
                           <div className="flex-1 min-w-0">
                              <p
                                 className={cn(
                                    ...
                                    : isCompleted ? "text-base-content/90"
                              ...
                              {isActive && (
                                 <p className="text-[10px] uppercase tracking-wider font-bold text-primary/70 mt-0.5">Sedang Dikerjakan</p>
                              )}
                           </div>
                           ...
                           <div className="flex items-center gap-2 shrink-0">
                              ...
                              {isActive && (
                                 <IconArrowRight size={16} className="text-primary transition-transform duration-300 group-hover:translate-x-1" />
                              )}
                           </div>
                        </button>
                     );
