import { z } from "zod";
import { extendZodWithOpenApi } from "@asteasolutions/zod-to-openapi";
extendZodWithOpenApi(z);
export class MaterialSchema {
    static MATERIAL_MODEL = z.object({
        id: z
            .uuid()
            .openapi({ example: "22222222-3333-4444-5555-666666666666" }),
        lessonId: z
            .string()
            .openapi({ example: "11111111-2222-3333-4444-555555555555" }),
        content: z
            .string()
            .min(10, "Konten materi terlalu singkat")
            .openapi({
            example: "<p>Kabel UTP adalah kabel yang digunakan untuk jaringan LAN...</p>",
        }),
        mediaUrl: z
            .url("Format URL media tidak valid")
            .nullable()
            .optional()
            .openapi({ example: "https://youtube.com/watch?v=contoh" }),
    });
    static CREATE_MATERIAL_REQUEST = this.MATERIAL_MODEL.omit({
        id: true,
        lessonId: true,
    }).strict();
    static UPDATE_MATERIAL_REQUEST = this.CREATE_MATERIAL_REQUEST.partial().strict();
    static MATERIAL_ID_PARAM = z.object({
        id: z.uuid("Format ID material tidak valid!"),
    });
}
//# sourceMappingURL=material.schema.js.map