...
import { Plus, Trash2, Edit2, CheckCircle2, HelpCircle, Link as LinkIcon } from "lucide-react";
import toast from "react-hot-toast";

const questionFormSchema = z.object({
	id: z.string().optional(),
	type: z.enum(["MULTIPLE_CHOICE", "COMMAND_TYPING", "CALCULATION_INPUT", "MATCHING", "IMAGE_LABELING", "SORTING"]),
	questionText: z.string().min(5, "Pertanyaan terlalu pendek"),
	xpReward: z.coerce.number().min(1, "XP harus lebih dari 0"),
	options: z.array(
		z.object({
			id: z.string().optional(),
	...
	if (data.type === "MULTIPLE_CHOICE") {
		...
		if (!data.matchingPairs || data.matchingPairs.length < 2) {
			ctx.addIssue({
				code: z.ZodIssueCode.custom,
				message: "Kuis menjodohkan minimal memiliki 2 pasang jawaban",
				path: ["matchingPairs"],
			});
		}
...
export function QuestionBuilder({ lessonId, questions = [] }: { lessonId: string, questions: any[] }) {
	const queryClient = useQueryClient();
	const [isModalOpen, setIsModalOpen] = useState(false);
	const [editingId, setEditingId] = useState<string | null>(null);

	const { register, control, handleSubmit, reset, setValue, formState: { errors } } = useForm<QuestionFormValues>({
	...
	const handleTypeChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
		...
		if (newType === "MULTIPLE_CHOICE") {
			...
			// Tipe input langsung (1 jawaban benar saja)
			setValue("options", [
				{ optionText: "", isCorrect: true },
			]);
		}
	...
	const { mutate: createMutation, isPending: isCreating } = useMutation({
		...
		onError: (error: any) => toast.error(error?.response?.data?.message || "Gagal menambahkan soal"),
	});

	const { mutate: updateMutation, isPending: isUpdating } = useMutation({
		mutationFn: ({ id, data }: { id: string, data: any }) => lessonApi.updateQuestion({ id, data }),
		onSuccess: () => {
			toast.success("Soal berhasil diperbarui!");
			closeModal();
			queryClient.invalidateQueries({ queryKey: ["admin-lesson", lessonId] });
		},
		onError: (error: any) => toast.error(error?.response?.data?.message || "Gagal memperbarui soal"),
	});
	...
	const onSubmit = (data: QuestionFormValues) => {
		...
		if (data.type === "MATCHING" && data.matchingPairs) {
			...
			payload.answerPattern = data.matchingPairs.map((_, i) => [i, i + numPairs]);
			payload.options = [];
		} else if ((data.type === "IMAGE_LABELING" || data.type === "SORTING") && data.imageLabels) {
	...
	const openModal = (question?: any) => {
		if (question) {
			...
			if (question.type === "MATCHING" && question.options) {
				const answerKeyOpt = question.options.find((opt: any) => opt.isCorrect);
				if (answerKeyOpt) {
					try {
						const pattern = JSON.parse(answerKeyOpt.optionText);
						matchingPairs = pattern.map((pair: string[]) => {
							const leftOpt = question.options.find((o: any) => o.id === pair[0]);
							const rightOpt = question.options.find((o: any) => o.id === pair[1]);
							return {
			...
			let imageLabels = [{value: ""}, {value: ""}];
			if ((question.type === "IMAGE_LABELING" || question.type === "SORTING") && question.options) {
				const answerKeyOpt = question.options.find((opt: any) => opt.isCorrect);
				if (answerKeyOpt) {
					try {
						const pattern = JSON.parse(answerKeyOpt.optionText) as number[];
		...
		<div className="space-y-4">
			<div className="flex items-center justify-between">
				<div>
					...
					<p className="text-sm text-base-content/60">Kelola soal latihan untuk materi ini.</p>
				</div>
			...
			<div className="grid gap-4 mt-4">
				{questions.length === 0 ? (
					<div className="text-center py-10 bg-base-200/50 rounded-xl border border-dashed border-base-300">
						...
						<p className="text-base-content/60">Belum ada soal kuis.</p>
					</div>
				) : (
					questions.map((q, idx) => (
						<div key={q.id} className="card bg-base-100 shadow-sm border border-base-200 hover:border-primary/30 transition-colors">
							<div className="card-body p-4 sm:p-6">
								<div className="flex justify-between items-start gap-4">
									<div>
										<div className="flex items-center gap-2 mb-2">
											<div className="badge badge-primary">Soal {idx + 1}</div>
											<div className="badge badge-ghost text-xs">{q.type}</div>
										</div>
										<h4 className="font-semibold text-lg">{q.questionText}</h4>
										<p className="text-xs text-base-content/60 mt-1">XP Reward: {q.xpReward}</p>
									</div>
									<div className="flex items-center gap-2">
										<button onClick={() => openModal(q)} className="btn btn-square btn-sm btn-ghost text-info">
											<Edit2 className="w-4 h-4" />
										</button>
										<button 
											onClick={() => {
												if(confirm("Yakin ingin menghapus soal ini?")) deleteMutation(q.id)
											}} 
											disabled={isDeleting}
								...
								<div className="mt-4">
									{q.type === "MULTIPLE_CHOICE" ? (
										<div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
											{q.options?.map((opt: any, oIdx: number) => (
												...
												>
													...
													{opt.isCorrect && <CheckCircle2 className="w-4 h-4 text-success shrink-0" />}
												</div>
											))}
										</div>
									) : q.type === "MATCHING" ? (
										<div className="grid grid-cols-1 md:grid-cols-2 gap-2">
											{(() => {
												const answerKeyOpt = q.options?.find((opt: any) => opt.isCorrect);
												if (answerKeyOpt) {
													try {
														const pattern = JSON.parse(answerKeyOpt.optionText);
														return pattern.map((pair: string[], i: number) => {
															const leftOpt = q.options.find((o: any) => o.id === pair[0]);
															const rightOpt = q.options.find((o: any) => o.id === pair[1]);
															return (
																<div key={i} className="flex items-center p-2 rounded-lg border border-base-200 bg-base-100/50 text-sm">
																	<span className="flex-1 text-center font-medium">{leftOpt?.optionText || "?"}</span>
																	<LinkIcon className="w-4 h-4 text-base-content/30 mx-2 shrink-0" />
																	<span className="flex-1 text-center font-medium">{rightOpt?.optionText || "?"}</span>
																</div>
															);
														});
													} catch (e) {}
										...
										<div className="p-3 rounded-lg border bg-success/10 border-success/30 text-success-content text-sm flex items-center gap-2">
											<div className="badge badge-success badge-sm">Kunci Jawaban</div>
											<span className="flex-1 font-mono">{q.options?.[0]?.optionText || "-"}</span>
						...
						<form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
							<div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
								<div className="form-control">
									<label className="label"><span className="label-text font-bold text-base-content">Tipe Soal</span></label>
									<select 
									...
									>
										...
										<option value="IMAGE_LABELING">Label Gambar (Drag & Drop)</option>
										<option value="SORTING">Urutkan Item (Sorting)</option>
									</select>
								</div>
								<div className="form-control">
									<label className="label"><span className="label-text font-bold text-base-content">XP Reward</span></label>
									<input 
										type="number" 
										{...register("xpReward")} 
										className="input input-bordered w-full bg-base-200/50" 
									/>
									{errors.xpReward && <span className="text-error text-xs mt-1">{errors.xpReward.message}</span>}
								</div>
							</div>

							<div className="form-control">
								<label className="label"><span className="label-text font-bold text-base-content">Pertanyaan</span></label>
								<textarea 
									{...register("questionText")} 
									className="textarea textarea-bordered h-24 bg-base-200/50" 
									placeholder="Tulis pertanyaan di sini..." 
								/>
								{errors.questionText && <span className="text-error text-xs mt-1">{errors.questionText.message}</span>}
							</div>

							<div className="divider text-sm font-bold text-base-content/40">Jawaban & Opsi</div>
							...
							{selectedType === "MULTIPLE_CHOICE" ? (
										...
										<div key={field.id} className="flex items-start gap-3">
											...
											<div className="flex-1">
												...
												{errors.options?.[index]?.optionText && (
													<span className="text-error text-xs mt-1 block">{errors.options[index]?.optionText?.message}</span>
												)}
											</div>
								...
								<div className="space-y-3 bg-base-200/30 p-4 rounded-xl border border-base-200">
									<div className="grid grid-cols-2 gap-4 mb-2 px-1">
										<span className="text-xs font-bold text-base-content/70 text-center">Sisi Kiri (Pertanyaan)</span>
										<span className="text-xs font-bold text-base-content/70 text-center">Sisi Kanan (Jawaban)</span>
									</div>
									{pairFields.map((field, index) => (
										<div key={field.id} className="flex items-start gap-2">
											<div className="flex-1">
												...
												/>
												{errors.matchingPairs?.[index]?.left && (
													<span className="text-error text-xs mt-1 block">{errors.matchingPairs[index]?.left?.message}</span>
												)}
											</div>
											...
											<div className="flex-1">
												...
												/>
												{errors.matchingPairs?.[index]?.right && (
													<span className="text-error text-xs mt-1 block">{errors.matchingPairs[index]?.right?.message}</span>
												)}
											</div>
											{pairFields.length > 2 && (
												<button 
							...
							) : (selectedType === "IMAGE_LABELING" || selectedType === "SORTING") ? (
								<div className="space-y-3 bg-base-200/30 p-4 rounded-xl border border-base-200">
									<div className="mb-2 px-1">
										<span className="text-xs font-bold text-base-content/70">Daftar Label (Susun sesuai urutan yang benar dari Atas ke Bawah)</span>
									</div>
										...
										<div key={field.id} className="flex items-center gap-2">
											...
											<div className="flex-1">
												...
												{errors.imageLabels?.[index]?.value && (
													<span className="text-error text-xs mt-1 block">{errors.imageLabels[index]?.value?.message}</span>
												)}
											</div>
								...
								<div className="bg-success/5 p-4 rounded-xl border border-success/20">
									<label className="label pt-0"><span className="label-text font-bold text-success-content">Kunci Jawaban Tepat</span></label>
									<input 
										{...register(`options.0.optionText` as const)} 
										className="input input-bordered w-full border-success font-mono"
										placeholder={selectedType === "COMMAND_TYPING" ? "Contoh: ping 192.168.1.1" : "Contoh: 62"}
									/>
									{errors.options?.[0]?.optionText && (
										<span className="text-error text-xs mt-1 block">{errors.options[0]?.optionText?.message}</span>
									)}
							...
							{errors.options?.root && (
								<div className="text-error text-sm text-center bg-error/10 p-2 rounded-lg">{errors.options.root.message}</div>
							)}
							{errors.matchingPairs?.root && (
								<div className="text-error text-sm text-center bg-error/10 p-2 rounded-lg">{errors.matchingPairs.root.message}</div>
							)}
							...
							<div className="modal-action mt-6 pt-4 border-t border-base-200">
								...
								<button type="submit" className="btn btn-primary px-8" disabled={isCreating || isUpdating}>
									{(isCreating || isUpdating) ? <span className="loading loading-spinner"></span> : "Simpan Soal"}
								</button>
							</div>
						</form>
					</div>
					<div className="modal-backdrop bg-base-300/80" onClick={closeModal}></div>
				</div>
			)}
		</div>
	);
