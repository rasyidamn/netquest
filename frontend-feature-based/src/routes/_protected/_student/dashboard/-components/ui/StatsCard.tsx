...
import { HeartIcon, StarIcon, ZapIcon } from "lucide-react";
...
function StatItem({ icon, label, value, theme }: StatItemProps) {
   ...
   return (
      <div className="group relative overflow-hidden rounded-2xl bg-base-100/40 backdrop-blur-md border border-white/5 p-4 sm:p-5 transition-all duration-500 hover:-translate-y-1 hover:bg-base-100/60 hover:shadow-lg z-10">
         ...
         <div className={cn("absolute -top-6 -right-6 h-16 w-16 rounded-full blur-xl transition-transform duration-500 group-hover:scale-150", glowStyles[theme])} />
...
export function StatsCard({ isLoading, xp, hearts }: StatsCardProps) {
   ...
   return (
      <div className="card relative overflow-hidden bg-base-200/40 backdrop-blur-xl border border-white/10 shadow-lg group">
         ...
         <div className="absolute top-0 left-0 w-full h-0.5 bg-linear-to-r from-transparent via-base-content/20 to-transparent opacity-50" />
         ...
         <div className="card-body p-6">
            ...
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
               ...
               <StatItem
                  theme="error"
                  icon={<HeartIcon size={24} className="animate-[pulse_2s_ease-in-out_infinite]" />}
                  label="Nyawa"
                  value={hearts ?? 0}
               />
            </div>
