...
export class LessonController {
	static getLessonByModule = catchAsync(
		async (req: Request, res: Response) => {
			...
			sendSuccess(
				res,
				StatusCodes.OK,
				"Berhasil mengambil daftar lesson",
				responseData,
			);
		},
	);

	static getLessonDetail = catchAsync(async (req: Request, res: Response) => {
		const params = ModuleSchema.MODULE_ID_PARAM.parse(req.params);
		const role = req.user?.role;
		const responseData = await LessonService.getLessonDetail(
			params.id,
			role as string,
		);
		sendSuccess(
			res,
			StatusCodes.OK,
			"Berhasil mengambil detail lesson",
			responseData,
		);
	});
	...
	static createLesson = catchAsync(async (req: Request, res: Response) => {
		const params = ModuleSchema.MODULE_ID_PARAM.parse(req.params);
		const validatedData = LessonSchema.CREATE_LESSON_REQUEST.parse(
			req.body,
		);
		const responseData = await LessonService.createLesson(
			params.id,
			validatedData,
		);
		sendSuccess(
   ...
   static deleteLesson = catchAsync(async (req: Request, res: Response) => {
	...
	static upsertMaterial = catchAsync(async (req: Request, res: Response) => {
		const params = LessonSchema.LESSON_ID_PARAM.parse(req.params);
		const validatedData = MaterialSchema.CREATE_MATERIAL_REQUEST.parse(
			req.body,
		);
		const responseData = await LessonService.upsertMaterial(
			params.id,
			validatedData,
		);
		sendSuccess(
	...
	static createQuestionWithOption = catchAsync(
		async (req: Request, res: Response) => {
			const params = LessonSchema.LESSON_ID_PARAM.parse(req.params);
			const validatedData =
				QuestionSchema.CREATE_QUESTION_WITH_OPTIONS_REQUEST.parse(
					req.body,
				);
			const responseData = await LessonService.createQuestionWithOption(
				params.id,
				validatedData,
			);
			sendSuccess(
   ...
   static updateQuestionWithOption = catchAsync(async (req: Request, res: Response) => {
