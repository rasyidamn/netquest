...
import { LessonTypeEnum, ProgressStatusEnum, QuestionType, } from "../generated/prisma/enums.js";
export class GameplayService {
     ...
     */
    static syncUserHearts = async (userId) => {
        ...
        const newUpdatedAt = new Date(user.heartsUpdatedAt.getTime() + heartsToRecover * cooldownPeriod);
    ...
    static theoryDone = async (userId, lessonId) => {
        ...
        if (existingProgress && existingProgress.status === ProgressStatusEnum.COMPLETED) {
            throw new ApiError(StatusCodes.BAD_REQUEST, "Anda sudah menyelesaikan materi ini!");
        }
        const result = await prisma.$transaction(async (tx) => {
            ...
            if (!user)
                throw new ApiError(StatusCodes.UNAUTHORIZED, "Sesi tidak valid. Silakan login kembali.");
    ...
    static submitQuiz = async (userId, validatedData) => {
        ...
        if (!question || question.lessonId !== validatedData.lessonId) {
            throw new ApiError(StatusCodes.BAD_REQUEST, "Data pertanyaan tidak valid atau tidak cocok dengan materi saat ini.");
        }
        let isAnswerCorrect = false;
        switch (question.type) {
            case QuestionType.MULTIPLE_CHOICE:
                const selectedOption = question.options.find((opt) => opt.id === validatedData.answer);
            ...
            case QuestionType.CALCULATION_INPUT:
                const correctTextOption = question.options.find((opt) => opt.isCorrect);
            ...
            case QuestionType.IMAGE_LABELING:
                ...
                const correctSortOption = question.options.find((opt) => opt.isCorrect);
                if (correctSortOption) {
                    try {
                        // Parsing jawaban frontend dan database untuk memastikan strukturnya sama persis
                        const expectedArray = JSON.parse(correctSortOption.optionText);
                        const submittedArray = JSON.parse(validatedData.answer);
                        // Cara instan membandingkan dua array string di JavaScript
                        if (JSON.stringify(expectedArray) ===
                            JSON.stringify(submittedArray)) {
                            isAnswerCorrect = true;
                        }
                    }
                    catch (error) {
                        throw new ApiError(StatusCodes.BAD_REQUEST, "Format array jawaban tidak valid.");
                    }
                }
                break;
            case QuestionType.TOPOLOGY:
                const correctTopologyOption = question.options.find((opt) => opt.isCorrect);
                if (correctTopologyOption) {
                    try {
                        // Frontend akan mengirimkan array of edges/connections, misalnya: ["nodeA-nodeB", "nodeC-nodeD"]
                        const expectedEdges = JSON.parse(correctTopologyOption.optionText);
                        const submittedEdges = JSON.parse(validatedData.answer);
                        ...
                        throw new ApiError(StatusCodes.BAD_REQUEST, "Format jawaban topologi tidak valid.");
                    }
                }
                break;
            default:
                throw new ApiError(StatusCodes.NOT_IMPLEMENTED, "Tipe kuis ini belum didukung.");
        }
                ...
                if (!isAlreadyCompleted) {
                    ...
                    await tx.user.update({
                        ...
                        data: {
                            ...
                            heartsUpdatedAt: currentUser.hearts === 3
    ...
    static completeQuiz = async (userId, validatedData) => {
        ...
        if (!lesson || lesson.type !== "QUIZ") {
            throw new ApiError(StatusCodes.BAD_REQUEST, "Pelajaran tidak valid.");
        }
        const result = await prisma.$transaction(async (tx) => {
            // 1. Cek User
