...
export class ProgressService {
    static getMyProgress = async (userId) => {
        ...
        return modules.map((module) => {
            ...
            let moduleStatus = ProgressStatusEnum.LOCKED;
            const hasActiveLesson = userProgresses.some((p) => p.status === ProgressStatusEnum.ACTIVE);
            const isAllLessonsCompleted = lessons.length > 0 &&
                lessons.every((l) => l.userProgress.some((p) => p.status === ProgressStatusEnum.COMPLETED));
            ...
            if (moduleStatus === ProgressStatusEnum.COMPLETED) {
                ...
                if (activeProgress) {
                    ...
                    const firstIncompleteLesson = lessons.find(l => !l.userProgress.length || l.userProgress[0]?.status !== ProgressStatusEnum.COMPLETED);
