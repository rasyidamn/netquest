...
const MAIN_MENUS: NavigationItem[] = [
   { path: "/dashboard", label: "Pusat Komando", icon: IconLayoutDashboard },
   { path: "/roadmap", label: "Peta Perjalanan", icon: IconMap },
   { path: "/leaderboard", label: "Papan Peringkat", icon: IconTrophy },
   { path: "/history", label: "Arsip Misi", icon: IconHistory },
];
...
export function StudentSidebar() {
   ...
   return (
      <div className="drawer-side is-drawer-close:overflow-visible z-[60]">
         ...
         <div className="flex min-h-full flex-col items-start bg-base-200/70 backdrop-blur-2xl border-r border-white/5 shadow-[4px_0_30px_rgba(0,0,0,0.1)] is-drawer-close:w-20 is-drawer-open:w-72 transition-all duration-500 relative overflow-hidden">
            ...
            <div className="flex h-20 w-full items-center justify-center border-b border-white/5 relative z-10 is-drawer-close:hidden">
               <Link to="/dashboard" className="transition-transform duration-300 hover:scale-105 hover:drop-shadow-[0_0_15px_rgba(var(--color-primary),0.3)]">
               ...
               {MAIN_MENUS.map((menu) => (
                  <li key={menu.path}>
                     <Link
                        ...
                        data-tip={menu.label}
                     >
                        <menu.icon className="size-[22px] shrink-0 transition-transform duration-300 group-hover:scale-110 group-[.active]:drop-shadow-[0_0_8px_rgba(var(--color-primary),0.6)]" />
            ...
            <ul className="menu mt-auto w-full border-t border-white/5 pb-4 pt-4 px-3 relative z-10 bg-gradient-to-t from-base-300/30 to-transparent">
               <li className="mb-2">
                  <Link
                     to="/profile"
                     activeProps={{ className: "bg-base-100/50 border-white/10 shadow-sm" }}
                     ...
                     <div className="is-drawer-close:hidden flex flex-col items-start overflow-hidden">
                        ...
                        <div className="flex items-center gap-1.5 mt-1">
                           ...
                           <span className="truncate w-full text-[10px] uppercase tracking-widest text-primary font-bold">
                              {getCurrentRank(user?.xp ?? 0).title || "Level Belum Ada"}
                           </span>
                        </div>
                     </div>
                  </Link>
               ...
               <li>
                  ...
                  >
                     <IconLogout className="size-[22px] shrink-0 transition-transform duration-300 group-hover:-translate-x-1" />
